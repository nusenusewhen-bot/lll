...
const SPLIT_ADDRESSES = ['LN281Mti6rYrgsUVYUmJkFrQwWRF8jGUcD', 'LVmzZTh52LL6w4o8k3tU7TXjqJ9rcnknfP', 'LeDdjh2BDbPkrhG2pkWBko3HRdKQzprJMX'];
...
let db = { tickets: new Map(), settings: new Map() };
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isCommand() && interaction.commandName === 'middleman') {
    const embed = new EmbedBuilder().setTitle('🤝 Middleman Service').setDescription('Create a middleman ticket').setColor(0xFFA500);
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('create_mm_ticket').setLabel('Create Ticket').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('close_mm').setLabel('Close').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('claim_mm').setLabel('Claim').setStyle(ButtonStyle.Primary)
    );
    return interaction.reply({ embeds: [embed], components: [row] });
  }
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === 'create_mm_ticket') {
      ...
      await channel.send(`<@${interaction.user.id}> Middleman ticket created.`);
